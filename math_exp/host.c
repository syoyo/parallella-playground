...
int main(int argc, char *argv[]) {
  ...
  // e_load_group("e_math_test.srec", &dev, 0, 0, platform.rows,  platform.cols,
  // E_FALSE);

  // To test
  e_load_group("e_fast_exp_test.srec", &dev, 0, 0, platform.rows,
  ...
  for (i = 0; i < platform.rows; i++) {
    for (j = 0; j < platform.cols; j++) {
      ...
      e_start(&dev, i, j);
      usleep(WAIT_MICROSECONDS); // Wait enough time.
      ...
      if (i == 0 & j == 0) {
        ...
        if (result[0] != 1) {
          ...
          fprintf(stderr, "[exp] Relative diff: ave = %f, min = %f, max = %f\n",
