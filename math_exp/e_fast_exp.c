...
float expapprox(float val) {
  ...
  b = xu2.f;
  return xu.f * (0.509964287281036376953125f +
...
int main(void) {
  ...
  volatile unsigned int exp_ref_clocks = 0;
  {
    ...
    sprintf(outbuf + strlen(outbuf),
            "\nThe clock cycle count for \"expf()\" (reference) is %d.\n",
            exp_ref_clocks);
  }
  ...
  if (1) {
    ...
    // re_f4 = expapprox(in_exp) + expapprox(in_exp1) + expapprox(in_exp2) +
    // expapprox(in_exp3); //ret[0] + ret[1] + ret[2] + ret[3];
    time_c = e_ctimer_get(E_CTIMER_1);
    e_ctimer_stop(E_CTIMER_1);

    // re_f4 = out_exp_arr[0] + out_exp_arr[1] + out_exp_arr[2] + out_exp_arr[3]
    // + out_exp_arr[4] + out_exp_arr[5] + out_exp_arr[6] + out_exp_arr[7];
  ...
  if (1) {
    ...
    volatile float ret =
        out_exp_arr[0] + out_exp_arr[1] + out_exp_arr[2] + out_exp_arr[3];
    // re_f4 = out_exp_arr[0] + out_exp_arr[1] + out_exp_arr[2] + out_exp_arr[3]
    // + out_exp_arr[4] + out_exp_arr[5] + out_exp_arr[6] + out_exp_arr[7];
    ...
    // volatile float diff = fabsf(ref - my);
    sprintf(outbuf + strlen(outbuf),
            "\nThe clock cycle count for \"expapprox4()\" is %d (/4 = %d).\n",
            temp, temp / 4);
    // sprintf(outbuf+strlen(outbuf)  , "\ndiff: %d.\n", (int)diff);
  }
